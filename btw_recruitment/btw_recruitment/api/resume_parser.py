...
def extract_text_from_file(file_path):
	...
	raise Exception(f"Unsupported file type '{mime}'. Upload PDF, DOCX, TXT, Image, or ZIP.")
...
def process_resume(docname):
	...
	try:
		...
		frappe.throw(f"Text extraction failed: {e!s}")
	...
	valid_doc_fields = {df.fieldname for df in doc.meta.fields}

	for doc_field, data_key in mapping.items():
		if doc_field not in valid_doc_fields:
			debug(f"Skipping unknown DocField '{doc_field}' in mapping")
			continue
