...
def extract_text_from_file(file_path, debug_log=None):
			...
			for i, p in enumerate(reader.pages):
				try:
					...
					debug_log.append(
						{
							"step": "PDF_EXTRACTION",
							"message": f"Page {i + 1}: Extracted {len(page_text)} characters",
						}
					)
				except Exception as e:
					debug_log.append(
						{"step": "PDF_EXTRACTION", "message": f"Page {i + 1}: Error - {e!s}", "error": True}
					)
			extracted_length = len(text.strip())
			debug_log.append(
				{
					"step": "PDF_EXTRACTION",
					"message": f"Total extracted: {extracted_length} characters",
					"success": True,
				}
			)
			return text.strip()
		except Exception as e:
			error_msg = f"PDF extraction failed: {e!s}"
	...
	if (
		...
		debug_log.append(
			{"step": "DOCX_EXTRACTION", "message": "Starting DOCX text extraction using Mammoth"}
		)
		try:
			with open(file_path, "rb") as docx_file:
				...
				debug_log.append(
					{
						"step": "DOCX_EXTRACTION",
						"message": f"Extracted {extracted_length} characters",
						"preview": preview,
						"success": True,
					}
				)
			...
			error_msg = f"DOCX extraction failed: {e!s}"
	...
	if mime == "text/plain":
		...
		try:
			text = open(file_path, encoding="utf-8", errors="ignore").read().strip()
			extracted_length = len(text)
			debug_log.append(
				{
					"step": "TXT_EXTRACTION",
			...
			error_msg = f"TXT extraction failed: {e!s}"
	...
	if mime and mime.startswith("image/"):
		...
		try:
			...
			debug_log.append(
				{
					"step": "IMAGE_OCR",
					"message": f"OCR extracted {extracted_length} characters",
					"success": True,
				}
			)
			return text.strip()
		except Exception as e:
			error_msg = f"Unable to extract text from image resume (OCR failed): {e!s}"
	...
	if mime == "application/zip" or file_path.endswith(".zip"):
		...
		try:
			...
			with zipfile.ZipFile(file_path, "r") as z:
				...
				debug_log.append(
					{"step": "ZIP_EXTRACTION", "message": f"ZIP contains {len(file_list)} files"}
				)
			...
			if "No readable resume" not in str(e):
				error_msg = f"ZIP extraction failed: {e!s}"
...
def process_resume(docname):
	...
	try:
		...
		debug_log.append(
			{"step": "FILE_LOOKUP", "message": f"Resume attachment URL: {doc.resume_attachment}"}
		)
		...
		if not os.path.exists(file_path):
			...
			frappe.throw(error_msg)
		debug_log.append(
			{"step": "FILE_VALIDATION", "message": f"File exists, size: {os.path.getsize(file_path)} bytes"}
		)
			...
			debug_log.append(
				{
					...
					"message": f"Text extraction completed: {extracted_length} characters",
					"success": True,
				}
			)
		except Exception as e:
			error_msg = f"Text extraction failed: {e!s}"
			debug_log.append(
				{
					"step": "TEXT_EXTRACTION",
					"message": error_msg,
		...
		debug_log.append(
			{
				...
				"message": f"Extracted text preview (first 200 chars): {extracted_text[:200]}",
...
"""
		debug_log.append(
			{
				"step": "AI_PARSING",
				"message": f"Sending request to Claude API (prompt length: {len(prompt)} chars)",
			}
		)
		...
		try:
			...
			debug_log.append(
				{"step": "AI_PARSING", "message": "Received response from Claude API", "success": True}
			)
		except Exception as e:
			error_msg = f"Claude API call failed: {e!s}"
			debug_log.append(
				{
					"step": "AI_PARSING",
					"message": error_msg,
		...
		debug_log.append(
			{
				"step": "AI_RESPONSE",
				"message": f"Raw AI response length: {len(json_text)} characters",
		...
		try:
			...
			debug_log.append(
				{
					"step": "JSON_PARSING",
					"message": f"JSON parsed successfully. Keys found: {list(data.keys())}",
					"success": True,
				}
			)
			...
			for f in list_fields:
				if isinstance(data.get(f), list):
					data[f] = ", ".join(data[f])
					debug_log.append(
						{"step": "DATA_TRANSFORM", "message": f"Converted {f} from list to string"}
					)
		except Exception as e:
			error_msg = f"Unable to read structured details from the resume. JSON parsing error: {e!s}"
			debug_log.append(
				{
					"step": "JSON_PARSING",
					"message": error_msg,
		...
		for field, key in mapping.items():
			if key in data:
				...
				debug_log.append(
					{"step": "FIELD_MAPPING", "message": f"Mapped {field} from {key}: {str(value)[:100]}"}
				)

		debug_log.append(
			{
				"step": "FIELD_MAPPING",
				"message": f"Mapped {len(mapped_fields)} fields",
		...
		try:
			...
			error_msg = f"Failed to save document: {e!s}"
			debug_log.append(
				{"step": "SAVE", "message": error_msg, "error": True, "traceback": traceback.format_exc()}
			)
			frappe.throw(error_msg)

		debug_log.append(
			{"step": "COMPLETE", "message": "Resume processing completed successfully", "success": True}
		)
		return {"status": "ok", "data": data, "debug_log": debug_log}

	except Exception as e:
		# Catch any unexpected errors
		error_msg = f"Unexpected error in process_resume: {e!s}"
		if "debug_log" in locals():
			debug_log.append(
				{"step": "ERROR", "message": error_msg, "error": True, "traceback": traceback.format_exc()}
			)
		else:
			debug_log = [
				{"step": "ERROR", "message": error_msg, "error": True, "traceback": traceback.format_exc()}
