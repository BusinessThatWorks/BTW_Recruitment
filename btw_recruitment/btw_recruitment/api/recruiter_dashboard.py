...
def get_recruiter_kpis(
	...
	jo_conditions = " AND jo.status IN ('Open', 'Closed – Hired')"  # noqa: RUF001
	jo_params = {}
	...
	if recruiter:
		jo_conditions += " AND jo.recruiter = %(recruiter)s"
		jo_params["recruiter"] = recruiter
...
def get_recruiter_openings(
	...
	conditions = "WHERE jo.status IN ('Open', 'Closed – Hired')"  # noqa: RUF001
	params = {}
	...
	if recruiter:
		conditions += " AND jo.recruiter = %(recruiter)s"
		params["recruiter"] = recruiter
...
def get_funnel_data(
	...
	if not openings:
		return {"mapping_stages": {}, "interview_stages": {}}
	...
	return {"mapping_stages": mapping_stages, "interview_stages": interview_stages}
