...
def execute(filters=None):
	...
	blacklisted_candidates = frappe.db.count("DKP_Candidate", {"blacklisted": "Yes"})
	...
	total_job_openings = frappe.db.count("DKP_Job_Opening", job_opening_filters)
	...
	for stage in stages:
		stage_filters = [["parenttype", "=", "DKP_Job_Opening"], ["stage", "=", stage]]

		if date_filter:
			stage_filters.append(["creation", *date_filter])

		count = frappe.db.count("DKP_JobApplication_Child", stage_filters)

		stage_counts.append(count)

	chart = {
		"data": {"labels": stages, "datasets": [{"name": "Candidates", "values": stage_counts}]},
		"type": "bar",
	}
