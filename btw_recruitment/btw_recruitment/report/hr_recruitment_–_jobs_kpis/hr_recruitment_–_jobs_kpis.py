...
def execute(filters=None):
	...
	rows = frappe.get_all("DKP_Job_Opening", fields=["number_of_positions"], filters=position_filters)
	...
	# }]
	STATUSES = ["Open", "On Hold", "Closed – Hired", "Closed – Cancelled"]  # noqa: RUF001
	...
	for status in STATUSES:
		...
		openings = frappe.db.count("DKP_Job_Opening", status_filters)

		rows = frappe.get_all("DKP_Job_Opening", fields=["number_of_positions"], filters=status_filters)

		positions = sum(int(r.number_of_positions or 0) for r in rows)

		status_cards.append({"status": status, "openings": openings, "positions": positions})

	data = [{"total_jobs": total_jobs, "total_positions": total_positions, "status_cards": status_cards}]
	...
	for status in statuses:
		...
		count = frappe.db.count("DKP_Job_Opening", chart_filters)
		status_counts.append(count)

	chart = {
		"data": {"labels": statuses, "datasets": [{"name": "Jobs", "values": status_counts}]},
		"type": "bar",
	}
