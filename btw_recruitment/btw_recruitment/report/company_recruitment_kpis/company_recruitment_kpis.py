...
def execute(filters=None):
	...
	active_clients = frappe.db.count("Customer", [*company_filters, ["custom_client_status", "=", "Active"]])

	# ---------------- KPI 3: Inactive Clients ----------------
	inactive_clients = frappe.db.count(
		"Customer", [*company_filters, ["custom_client_status", "=", "Inactive"]]
	)

	companies_with_open_jobs = 0

	if date_filter:
		companies_with_open_jobs = (
			frappe.db.sql(
				"""
	...
	chart_industry = {
		"data": {"labels": industry_labels, "datasets": [{"name": "Clients", "values": industry_values}]},
		"type": "bar",
	}
