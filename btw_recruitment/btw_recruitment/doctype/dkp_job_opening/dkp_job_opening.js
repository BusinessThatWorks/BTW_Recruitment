		...
		primary_action: () => {
			...
			candidates.forEach((candidate) => {
				if (selected_map[candidate.name]) {
					candidate_display_names[candidate.name] =
			...
			show_multiple_candidates_previous_openings(
				selected_candidates,
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <div><strong>Age:</strong> ${candidate.age || "-"}</div>
                                    ...
                                    <div><strong>Current CTC:</strong> ${
										candidate.current_ctc || "-"
									}</div>
                                    <div><strong>Expected CTC:</strong> ${
...
function show_multiple_candidates_previous_openings(
	candidate_names,
	candidate_display_names,
	current_job_opening,
	callback,
	...
	Promise.all(promises)
        ...
        `;
			...
			const days_used = results[0]?.message?.days_used || 7;
			// Create and show dialog
			const dialog = new frappe.ui.Dialog({
...
frappe.ui.form.on("DKP_JobApplication_Child", {
	candidate_name(frm, cdt, cdn) {
		...
		frappe.db
			.get_value("DKP_Candidate", row.candidate_name, [
				"blacklisted",
				...
				if (candidate.current_company_master) {
					frappe.db
						...
						.then((company_r) => {
							if (
								company_r.message &&
								company_r.message.custom_no_poach_flag === "Yes"
							) {
								const company_label =
									company_r.message.customer_name ||
									candidate.current_company_master;
								frappe.msgprint({
