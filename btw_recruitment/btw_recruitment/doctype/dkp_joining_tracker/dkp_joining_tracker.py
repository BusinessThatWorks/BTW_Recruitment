...
class DKP_Joining_Tracker(Document):
	def on_trash(self):
		...
		if self.interview_ref:
			frappe.db.set_value("DKP_Interview", self.interview_ref, "invoice_ref", None)
	...
	def is_frozen_due_to_replacement_policy(self):
		...
		replacement_policy = frappe.db.get_value("Customer", self.company_name, "custom_replacement_policy_")
...
def check_joining_tracker_freeze_status(tracker_name):
	...
	result = {"is_frozen": False, "message": ""}
	...
	replacement_policy = frappe.db.get_value("Customer", doc.company_name, "custom_replacement_policy_")
