frappe.pages["joining-tracker"].on_page_load = function (wrapper) {
	...
	function render_table(rows) {
		...
		// 🟢 Reordered columns - Billing info comes right after Status
		const columns = [
			{
				name: "Tracker ID",
				width: 160,
			...
			// 🟢 Billing columns moved here - right after Status
			{
				name: "Billing Status",
				width: 140,
				format: (v) => {
			...
			// 🟢 Other details moved to the end
		...
		// 🟢 Data array also reordered to match columns
		const data = rows.map((r) => [
			...
			r.joining_date || "-",
			r.status || "-",
			// 🟢 Billing data moved here
			r.billing_status || "Yet to Bill",
			r.billable_ctc || 0,
			r.billing_value || 0,
			r.billing_fee || "-",
			r.billing_month || "-",
			// 🟢 Other details at the end
			r.candidate_contact || "-",
			r.hiring_location || "-",
			r.recruiter || "-",
			r.recipients_name || "-",
